...
def evaluate_firstrewardtime(args,
             policy,
             ret_rms,
             iter_idx,
             tasks,
    ...
    if num_episodes is None:
        num_episodes = args.max_rollouts_per_task
    num_processes = 16 #args.num_processes
    ...
    num_steps = 50 #envs._max_episode_steps
    ...
    first_rew = [num_steps * num_episodes] * num_processes
    ...
    for episode_idx in range(num_episodes):
        for step_idx in range(num_steps):
            ...
            for i in range(num_processes):
                if rew_raw[i] > 0:
                    first_rew[i] = min(first_rew[i] , episode_idx * num_steps + step_idx)
...
def evaluate_firstrewardtimev2(args,
             policy,
             ret_rms,
             iter_idx,
             tasks,
...
def evaluate_searchtime(args,
    ...
    # --- initialise environments and latents ---
    num_steps = 100 #envs._max_episode_steps
    ...
    envs.close()
    print (first_rew)
    print (infos[0]['vis'].reshape(5, 5))
    return first_rew

def evaluate_searchtimev2(args,
             policy,
             ret_rms,
             iter_idx,
             tasks,
